...
@Component({
  selector: 'app-neue-fahrrad-list',
  standalone: true,
  ...
  template: `
    <div class="page">
      <div class="page-header">
        <div>
          <h1>{{ t.neueFahrraeder }}</h1>
          <p class="subtitle">{{ items.length }} {{ t.total }}</p>
        </div>
        <a routerLink="/neue-fahrraeder/new" class="btn btn-primary">
          + {{ t.neueFahrradNew }}
        </a>
      </div>
    ...
    if (this.searchTerm) {
      ...
      result = result.filter(
        (i) =>
          ...
          i.modell?.toLowerCase().includes(term) ||
